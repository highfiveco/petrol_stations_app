...
public class MultipartUtility {
     ...
     * @param headerkeys
     *@param headervalues
     * @param charset  @throws IOException
     */
    public MultipartUtility(String requestURL, ArrayList<String> headerkeys, ArrayList<String> headervalues, String charset)
            throws IOException {
        ...
        if (headerkeys != null && headervalues != null) {
            if (headerkeys.size() == headervalues.size()) {
                for (int i = 0; i < headerkeys.size(); i++) {
                    httpConn.setRequestProperty(headerkeys.get(i), headervalues.get(i));
                }
            }
        }
    ...
    */
     ...
     * @throws IOException
     */
    public ResponseObject finish() throws IOException {
        ...
        if (status == HttpURLConnection.HTTP_OK) {
            ...
            responseObject.setResponseText(response);
        } else {
//            throw new IOException("Server returned non-OK status: " + status);
            ...
            responseObject.setResponseText(inputStreamToString(in));
            responseObject.setResponseCode(status);

            Log.e("response",""+inputStreamToString(in));
        }
